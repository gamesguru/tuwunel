		...
		loop {
			...
			for i in 0..rejected_outliers.len() {
				...
				let auth_events: Vec<_> = event
					...
					.ready_filter_map(|auth_event| {
						...
						if let Some(state_key) = auth_event.state_key() {
							...
							warn!(?event_type, event_id = %auth_event.event_id(), "Auth event is missing state_key");
							None
						}
					})
					.collect()
				...
				if auth_pass {
					...
					let handle_res = Box::pin(self.handle_incoming_pdu(
						origin,
						room_id,
						event_id,
						pdu_json.clone(),
						true, // is_timeline_event
					))
					.await;
					...
					match handle_res {
						...
						| Err(e) => {
							warn!(?event_id, "Failed to promote unrejected event: {e}");
							return Err(e);
						},
					}
				}
